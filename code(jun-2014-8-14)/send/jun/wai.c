...
int   init_socket_server3();
int   init_socket_server4(char *confname);
int   init_socket_server1();
int   get_ip_port(char *pathname, char *ip, int *port);
void  *recv_file(void *arg);
int   recv_parse_file(int fd);
...
void *start_server1()
{
	...
	while(1)
	{
		...
		pthread_create(&tid3, NULL, recv_file, &connfd);
...
void *start_server3()
{
	...
	pthread_t	tid3;
        ...
        if((listenfd = init_socket_server3()) < 0)
        {
                printf("init_socket_server error [%d]\n", __LINE__);
        }
...
void *start_server2()
{
	...
	int		i = 0;
        ...
        if((listenfd = init_socket_server1()) < 0)
        {
                printf("init_socket_server error [%d]\n", __LINE__);
        }
...
void *start_server4()
{
	...
	int		i = 0;
        ...
        if((listenfd = init_socket_server4("/root/onesafe/code/jun/conf8")) < 0)
        {
                printf("init_socket_server error [%d]\n", __LINE__);
        }
...
int recv_parse_file(int fd)
{
        ...
        sprintf(createname, "%s-zhongxing", filename);
...
//	printf("request is %s, len = %d\n", request,len);
	...
	filename[strlen(filename)] = '\0';
	sprintf(filename, "%s-huiju", filename);
...
int init_socket_server1()
{
        ...
        if(get_ip_port("/root/onesafe/code/jun/conf9", ip, &port) < 0)
        {
                printf("get_ip_port error\n");
                return -1;
        }
